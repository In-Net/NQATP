...
def writer(d_type, input_data, isFirst: bool):
    with open("control-entry-usage-extended.csv", "a") as csvfile:
...
if __name__ == "__main__":
    # data gen: python3 gen_query-v3.py -m 4 -c 50 -a 5 -g -o /home/lthpc/git/catchment-basin-seeker/data/queries-v3-small.json
    # python3 gen_query-v3.py -p 256 -q 100000 -s 8 -o /home/lthpc/git/catchment-basin-seeker/data/queries-v3-8.json
    ...
    with open("/home/lthpc/git/catchment-basin-seeker/data/queries-v3-small.json") as input:
    ...
    print("Overall Bandwidth cost = %d" % (cost))

    switch_load_wi_cluster = [0] * 512
    switch_load_wo_cluster = [0] * 512
    switch_load_default = [0] * 512
    sw_in_pod = 16
    ...
    for idx in range(8):
        ...
        switch_load_default[idx] += (args.query * 8)
        switch_load_default[256+idx] += args.query
    # With cluster
        ...
        for index, pod in enumerate(product):
            if (pod > 0):
                for aggr_idx in range(8):
                    ...
                    switch_load_wi_cluster[256 + index*8 + aggr_idx] += 1
        ...
        for index, pod in enumerate(source):
            if (pod > 0):
                for aggr_idx in range(8):
                    ...
                    switch_load_wo_cluster[256 + index*8 + aggr_idx] += 1
                    switch_load_default[index*8 + aggr_idx] += 1
                    switch_load_default[256 + index*8 + aggr_idx] += 1

    # default require IP routing for response with dynamic routing
    for pod_idx in range(1, 32):
        for s_idx in range(8):
            switch_load_default[pod_idx*8 + s_idx] += 8
            switch_load_default[256+ pod_idx*8 + s_idx] += 1
    ...
    writer(d_type="default", input_data=switch_load_default, isFirst=False)
