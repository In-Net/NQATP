...
data_e2 = pd.read_csv('./data-e2-ratio.csv')
data_e3 = pd.read_csv('./data-e3-ratio.csv')
data_e4 = pd.read_csv('./data-e4-ratio.csv')
...
axs[1].legend(handles=axs[1].lines, labels=["3 Tier Polling","DriftSwitch (One Cluster)"], frameon=False, loc="center",  bbox_to_anchor=(0.5,1.2),  ncol=2, columnspacing=2, handletextpad=0.3, handlelength=1.5)
...
axs[0].xaxis.set_ticks(np.arange(0, 4.6, 0.9))
axs[1].xaxis.set_ticks(np.arange(0, 6.5, 1.6))
axs[2].xaxis.set_ticks(np.arange(0, 9, 2))
...
f.savefig("./origin-bmv2-link-ratio.pdf", bbox_inches='tight', pad_inches=0)
