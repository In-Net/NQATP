...
data_e2 = pd.read_csv('./data-e2-scale.csv')
data_e3 = pd.read_csv('./data-e3-scale.csv')
data_e4 = pd.read_csv('./data-e4-scale.csv')
...
plot0 = sns.ecdfplot(data_e2, x="Kpps", hue="Type", palette=my_pal, linewidth=2, ax=axs[0])
plot1 = sns.ecdfplot(data_e3, x="Kpps", hue="Type", palette=my_pal, linewidth=2, ax=axs[1])
plot2 = sns.ecdfplot(data_e4, x="Kpps", hue="Type", palette=my_pal, linewidth=2, ax=axs[2])
...
axs[0].xaxis.set_ticks(np.arange(0, 150, 20))
axs[1].xaxis.set_ticks(np.arange(0, 220, 30))
axs[2].xaxis.set_ticks(np.arange(0, 300, 40))
...
axs[1].set_xlabel("One-directional Link Usage (kPPS)", fontsize=14)
axs[1].set_ylabel("")
axs[2].set_xlabel("")
axs[2].set_ylabel("")
...
f.savefig("./origin-bmv2-link-scale.pdf", bbox_inches='tight', pad_inches=0)
