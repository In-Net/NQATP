...
#define CONTROL_HDRLEN 10         // Control header length
#define IPPROTO_MYCONTROL 0x9F    // IP proto for my control header
#define MAX_QUERY_ID 100
...
main (int argc, char **argv)
{
  ...
  struct ip iphdr;
  struct my_control_t my_control;
  ...
  iphdr.ip_len = htons (IP4_HDRLEN + CONTROL_HDRLEN);
  ...
  frame_length = 6 + 6 + 2 + IP4_HDRLEN + CONTROL_HDRLEN;
  ...
  for (unsigned int query_id = 0; query_id < MAX_QUERY_ID; query_id ++) {
    // My control header
    my_control.queryID = htons (query_id);
    my_control.flowCount = htons (2);
    my_control.seq = htons (1);

    // Next part of packet is upper layer protocol header.
    memcpy ((ether_frame + ETH_HDRLEN + IP4_HDRLEN), &my_control, CONTROL_HDRLEN);
